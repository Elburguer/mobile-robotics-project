...
def initialize_queues_processes():
    ...
    manager = mp.Manager()
    visual_pose_queue = manager.Queue(maxsize=100)       # Define appropriate maxsize    
    ...
    processes = [
        mp.Process(target=visual_perception_loop, daemon=True, args=(visual_pose_queue,visual_ready)),        
    ]

    for process in processes:
        process.start()

    return manager,visual_pose_queue, visual_ready,processes


def wait_for_data(visual_ready,pose_queue):
...
def loggin_stuff(visual_pose_plot,visual_pose,dt_plot,current_loop_time,last_control_time,t_plot,current_time, start_time,i):
